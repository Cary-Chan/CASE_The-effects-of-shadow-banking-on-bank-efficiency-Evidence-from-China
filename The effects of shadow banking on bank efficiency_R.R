...
library(plm) 
#Input data. 
data1<-read.table(file="/Users/chenchuandong/Desktop/thesis/input-output参考 献/data-MasterThesis.csv", header=TRUE, sep= ";", dec=".", na.string="NA") summary(data1) 
data2<-plm.data(data1,c("BankNum","YEAR")) summary(data2) 
tc<-data2$TC loans<-data2$LOAN la<-data2$L A oea<-data2$OEA dstf<-data2$DSTF pf1<-data2$PF1 pc<-data2$PC z1<-data2$Z1 z2<-data2$Z2 tea<-data2$TEA ta<-data2$TA wto<-data2$WTO cbrc<-data2$CBRC fc<-data2$FC year<-as.numeric(data2$YEAR) 
#Descriptive statistics of the outputs and inputs variables. 
mean(tc); max(tc); min(tc); sd(tc) mean(loans); max(loans); min(loans); sd(loans) 
mean(dstf); max(dstf); min(dstf); sd(dstf) 
...
+I(0.5*log(dstf/ta)^2) +I(0.5*log(pf1/pc)^2) +I(log(loans/ta)*log(dstf/ta))+I(log(loans/ta)*log(pf1/pc)) 
+I(log(dstf/ta)*log(pf1/pc)) +year +I(0.5*year^2) +I(log(loans/ta)*year)+I(log(dstf/ta)*year) +I(log(pf1/pc)*year) 
|z1+z2+wto+fc+cbrc+log(ta)+log(la/ta)+I(loans/dstf), data=data2,ineffDecrease=FALSE) 
summary(model_1) lrtest(model_1) 

model_1_0<-sfa(log(tc/(pc*ta))~log(loans/ta)+log(dstf/ta)+log(pf1/pc) +I(0.5*log(loans/ta)^2) 
+I(0.5*log(dstf/ta)^2) +I(0.5*log(pf1/pc)^2) +I(log(loans/ta)*log(dstf/ta))+I(log(loans/ta)*log(pf1/pc)) +I(log(dstf/ta)*log(pf1/pc)) +year +I(0.5*year^2) +I(log(loans/ta)*year)+I(log(dstf/ta)*year) +I(log(pf1/pc)*year) 
...
prec11<-(sum(h11<0)+sum(h11=0))/(sum(h11<0)+sum(h11=0)+sum(h11>0)) prec22<-(sum(h22<0)+sum(h22=0))/(sum(h22<0)+sum(h22=0)+sum(h22>0)) prec33<-(sum(h33<0)+sum(h33=0))/(sum(h33<0)+sum(h33=0)+sum(h33>0)) 
...
+I(0.5*log(dstf/ta)^2) +I(0.5*log(pf1/pc)^2) +I(log(loans/ta)*log(dstf/ta))+I(log(loans/ta)*log(pf1/pc)) +I(log(dstf/ta)*log(pf1/pc)) +year +I(0.5*year^2) 
+I(log(loans/ta)*year)+I(log(dstf/ta)*year) +I(log(pf1/pc)*year) |z1+z2, 
...
+I(0.5*log(dstf/tea)^2) +I(0.5*log(pf1/pc)^2) +I(log(loans/tea)*log(dstf/tea))+I(log(loans/tea)*log(pf1/pc)) +I(log(dstf/tea)*log(pf1/pc)) +year +I(0.5*year^2) +I(log(loans/tea)*year)+I(log(dstf/tea)*year) +I(log(pf1/pc)*year) 
|z1+z2+wto+fc+cbrc+log(ta)+log(la/ta)+I(loans/dstf), data=data2,ineffDecrease=FALSE) 
